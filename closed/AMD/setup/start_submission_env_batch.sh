...
export LAB_TS="${SLURM_JOB_ID:-manual}-$(date +%m%d-%H%M%S)"
...
docker run ${EXTRA_ARGS} --rm -i --ipc=host --network=host --privileged --cap-add=CAP_SYS_ADMIN --device=/dev/kfd --device=/dev/dri --device=/dev/mem \
